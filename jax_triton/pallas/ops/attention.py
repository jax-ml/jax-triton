...
def mha_forward_kernel(
  ...
  l_i = jnp.zeros(block_q, dtype=jnp.float32) - float('inf')
  # acc is the buffer where we accumulate the output on sram.
  ...
  acc *= jnp.exp(jnp.negative(l_i))[:, None]
...
def _mha_backward(sm_scale: float, causal: bool, block_q: int, block_k: int,
  ...
  block_q = min(block_q, seq_len)
  block_k = min(block_k, seq_len)
  delta = _preprocess_backward(out, do, block_q, debug, interpret)[0]
  ...
  if backward_pass_impl == "xla":
    ...
    dq, dk, dv = pl.pallas_call(
        ...
        num_warps=num_warps,
        num_stages=1,
        input_output_aliases={input_output_alias: 0})(q, k, v, bias, out, do, l, m, delta, dq)
  else:
...
def mha_reference(q, k, v, bias=None, sm_scale=1.0, causal: bool=False, has_bias: bool=False):
  ...
  if has_bias:
    weights = jax.nn.softmax(logits * sm_scale + bias.astype(logits.dtype)).astype(q.dtype)
  else:
