...
def compile_ttir_inplace(    
  ...
  if is_device_cuda():
    ...
    compilation_result = compile_ttir_to_hsaco_inplace(
          ttir,
          backend,
          options,
          compute_capability,
...
def compile_ttir_to_hsaco_inplace(
  ...
  amdgcn = hip_backend.make_amdgcn(
      llir,
      metadata,
      hip_options
  )

  hsaco = hip_backend.make_hsaco(
    amdgcn,
    metadata,
    hip_options
  ) 
  ...
  ttgir = str(ttgir) if _JAX_TRITON_DUMP_DIR else None
  llir = str(llir) if _JAX_TRITON_DUMP_DIR else None
  cluster_dims = (0,0,0) #cluster dims are NOT useful on hip backend. We just fill up with some value for API compatibility
  fd, hsaco_path = tempfile.mkstemp() #Write hsaco to a file and then pass the path
