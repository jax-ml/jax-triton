...
class PallasCallTest(PallasTest):
      ...
      *[
          (
              (
                  ...
                  f"bn_{block_size_n}_bk_{block_size_k}_gm_{group_size_m}"
              ),
              m,
              n,
              k,
  ...
  def test_matmul(self, m, n, k, dtype, bm, bn, bk, gm):
    ...
    if jt.get_compute_capability(0) <= 75 and (bm > 128 or bn > 128 or bk > 32):
    ...
    out, expected = matmul(
        x, y, bm=bm, bn=bn, bk=bk, gm=gm, interpret=self.INTERPRET
    ), jnp.matmul(x, y)
  ...
  @parameterized.named_parameters(
      *[
          (
              (
                  f"m_{m}_n_{n}_k_{k}_dtype_{dtype}_bm_{block_size_m}_"
                  f"bn_{block_size_n}_bk_{block_size_k}"
              ),
              m,
              n,
              k,
  ...
  def test_matmul_block_spec(self, m, n, k, dtype, bm, bn, bk):
    ...
    x = random.normal(k1, (m, k), dtype=dtype)
    y = random.normal(k2, (k, n), dtype=dtype)
    out, expected = matmul_block_spec(
        x, y, bm=bm, bn=bn, bk=bk, interpret=self.INTERPRET
    ), jnp.matmul(x, y)
    np.testing.assert_allclose(out, expected, atol=0.05, rtol=0.05)
  ...
  def test_masked_load_store(self, size, block_size):
    ...
    np.testing.assert_allclose(add_one(x), x + 1.0, atol=1e-5, rtol=1e-5)
  ...
  def test_broadcasted_load_store(self):
    ...
    def load(x_ref, o_ref):
      ...
      pl.store(o_ref, (jnp.arange(m), jnp.arange(n)), x + 1.0)
  ...
  def test_using_pallas_slice(self):
    ...
    @functools.partial(self.pallas_call, out_shape=out_shape, grid=1)
...
class PallasControlFlowTest(PallasTest):
  ...
  def test_loop_with_float64_carry(self):
    ...
    try:
      ...
      np.testing.assert_allclose(
          np.arange(1, 5.0) * 3, f(jnp.arange(1, 5.0, dtype=jnp.float64))
      )
    finally:
  ...
  def test_cond_simple(self):
    ...
    def f(branch_ref, x_ref, y_ref):
      y_ref[...] = lax.switch(
          branch_ref[...], (lambda x: x**2, lambda x: -x), x_ref[...]
      )
  ...
  def test_conditional_write(self):
    ...
    np.testing.assert_allclose(
        f(jnp.bool_(True), arg), jnp.float32([0.0, -1, -2, -3, 6, -5, -6, -7])
    )
  ...
  def test_scan_cond_vm_explicit_ref_arg(self):
    ...
    params = jnp.arange(len(program) * 3.0).reshape(len(program), 3)
    x = jnp.arange(7.0)
    bx = 4
    ...
    def f(program_ref, params_ref, x_ref, out_ref):
      ...
      def body_fn(i, args):
        ...
        state = jax.lax.switch(
            opcode,
            (
                lambda state, params, i: state + params[i, 0] * 2.0**i * x,
                lambda state, params, i: state + params[i, 1] * 2.0**i * x,
                lambda state, params, i: state + params[i, 2] * 2.0**i * x,
                lambda state, params, i: state + params[i, 1] * 2.0**i * x,
            ),
            state,
            params_ref,
      ...
      out_ref[...] = jax.lax.fori_loop(
          ...
          len(program),
          body_fn,
          (jnp.zeros(x.shape), program_ref, params_ref),
      )[0]
      ...
      def body_fn(i, args):
        ...
        state = jax.lax.switch(
            ...
            (
                ...
                lambda state, i: state + params_ref[i, 0] * 2.0**i * x,
                lambda state, i: state + params_ref[i, 1] * 2.0**i * x,
                lambda state, i: state + params_ref[i, 2] * 2.0**i * x,
                lambda state, i: state + params_ref[i, 1] * 2.0**i * x,
            ),
            state,
            i,
...
class PallasCallAutodifferentiationTest(PallasTest):
  ...
  def test_jvp(self, impl):
    ...
    np.testing.assert_allclose(
        out_tangent, out_tangent_ref, atol=1e-5, rtol=1e-5
    )
...
class PallasCallVmapTest(PallasTest):
  ...
  def test_quadruple_vmap_of_batched_kernel(self):
    @functools.partial(
        self.pallas_call,
        out_shape=jax.ShapeDtypeStruct((7,), jnp.int32),
        debug=False,
        grid=(7,),
    )
    def add_one(x_ref, o_ref):
      ...
      *[
          (
              (
                  ...
                  f"_{use_fwd=} _{use_segment_ids=}_{use_segment_ids=}"
              ),
              batch_size,
              seq_len,
              num_heads,
